...
- **Employee schedule view.** Read-only users load schedules from `GET /api/schedule/{weekId}/employee-view`, which returns only schedule days, segment blocks, staff assignments, and employee names (no contact info or settings).
