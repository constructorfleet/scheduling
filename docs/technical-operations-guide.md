...
- **Integration**: Rendering `App` and calling `handleAddClockBlock` (`src/ui/App.tsx:286`) together should mutate `segmentBlocksState`/`staffAssignmentsState`, rerun the rule engine (`src/ui/App.tsx:128`), and feed `violationRecords` plus `GuidedStatusTracker` so guardrail violations drop from the navigator when expectations align. Use the weekday mock data (`src/ui/data/mockScheduleData.ts:263`) that already includes non-contiguous Monday blocks, add an in-window block, confirm the timeline renders the new segment, the guardrail badge disappears, and the tracker steps turn “Complete.”
