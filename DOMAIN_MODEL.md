...
- **Time segment coverage**: For clock windows, the aggregated `StaffAssignment`s must meet the configured requirements for that day’s `DayOfWeek`, `ScheduleType`, and `DayScheduleType`; blocking violations mention the missing role, overlapping coverage, or missing schdule block and reference the offending `SegmentBlock`.
