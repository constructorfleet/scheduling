...
WITH
    fallback AS (
        SELECT DISTINCT
            ON (jt."schoolId") jt."schoolId",
            jt."id",
            jt."title"
        FROM
            "JobTitle" jt
        WHERE
            LOWER(TRIM(jt."title")) <> 'staff'
        ORDER BY
            jt."schoolId",
            jt."title"
    )
...
FROM
    fallback
WHERE
    fallback."schoolId" = e."schoolId"
    AND (
        e."jobTitleId" IS NULL
        OR LOWER(TRIM(e."jobTitle")) = 'staff'
    );

-- Remove Staff job titles from the catalog
DELETE FROM "JobTitle"
WHERE
    LOWER(TRIM("title")) = 'staff';

-- Enforce NOT NULL on jobTitleId
ALTER TABLE "Employee"
ALTER COLUMN "jobTitleId"
SET
    NOT NULL;