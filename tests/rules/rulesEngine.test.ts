...
describe("RulesEngine", () => {
    ...
    test("applies field-trip ratios as overrides for segment ratio checks", () => {
        ...
        expect(tripViolations.every((v) => v.target.id === defaultScheduleDay.id)).toBe(true);
    });
    ...
    test("passes field trip ratios when adult and leader counts meet minima", () => {
        ...
        const context: RulesContext = withDefaultScheduleInfo({
            scheduleDays: [
                createScheduleDay("day-trip-clean-thu", {
                    dayOfWeek: "thu",
                    fieldTripEventId: fieldTripEvent.id
                })
            ],
    ...
    test("reports leader shortages when adult counts satisfy field-trip minima", () => {
        ...
        const context: RulesContext = withDefaultScheduleInfo({
            scheduleDays: [
                createScheduleDay("day-trip-shortage-thu", {
                    dayOfWeek: "thu",
                    fieldTripEventId: fieldTripEvent.id
                })
            ],
    ...
    test("flags assignments with invalid time windows", () => {
        const invalidBlock = createSegmentBlock("block-invalid-window", {
            startTime: "12:00",
            endTime: "10:00"
        });
        const invalidAssignment = createAssignment("assign-invalid-window", invalidBlock.id, "emp-invalid-window", {
            startTime: "12:00",
            endTime: "10:00"
        });
        const context: RulesContext = withDefaultScheduleInfo({
            segmentBlocks: [ invalidBlock ],
            staffAssignments: [ invalidAssignment ],
            employees: [ createEmployee("emp-invalid-window") ],
            fieldTripEvents: [],
            fieldTripTypes: []
        });
        ...
        expect(timelineViolations[ 0 ].target.id).toBe(invalidAssignment.id);
        expect(timelineViolations[ 0 ].target.metadata).toEqual({
            startTime: invalidAssignment.startTime,
            endTime: invalidAssignment.endTime,
            dayOfWeek: "mon",
            relatedSegmentBlockIds: [ invalidBlock.id ]
        });
    ...
    test("flags overlapping assignments for the same employee/day", () => {
        ...
        expect(timelineViolations[ 0 ].message).toContain("overlapping schdule blocks");
    });

    test("flags assignments that start before operating hours", () => {
        ...
        const earlyAssignment = createAssignment("assign-before-open", earlyBlock.id, "emp-before-open", {
            startTime: "06:30",
            endTime: "09:00"
        });
        const context: RulesContext = withDefaultScheduleInfo({
            segmentBlocks: [ earlyBlock ],
            staffAssignments: [ earlyAssignment ],
            employees: [ createEmployee("emp-before-open") ],
            fieldTripEvents: [],
            fieldTripTypes: [],
        ...
        expect(timelineViolations[ 0 ].target.metadata).toEqual({
            ...
            startTime: earlyAssignment.startTime,
            boundary: customHours.open,
            dayOfWeek: "mon",
            relatedSegmentBlockIds: [ earlyBlock.id ]
        });
    });

    test("flags assignments that end after operating hours", () => {
        const customHours = createOperatingHours({
            dayOfWeek: "mon",
            dayScheduleType: "full_day",
            open: "06:00",
        ...
        const lateAssignment = createAssignment("assign-after-close", lateBlock.id, "emp-after-close", {
            startTime: "10:00",
            endTime: "13:00"
        });
        const context: RulesContext = withDefaultScheduleInfo({
            segmentBlocks: [ lateBlock ],
            staffAssignments: [ lateAssignment ],
            employees: [ createEmployee("emp-after-close") ],
            fieldTripEvents: [],
            fieldTripTypes: [],
            operatingHours: [ customHours ]
        });
        ...
        expect(timelineViolations[ 0 ].target.metadata).toEqual({
            operatingHoursId: customHours.id,
            endTime: lateAssignment.endTime,
            boundary: customHours.close,
            dayOfWeek: "mon",
            relatedSegmentBlockIds: [ lateBlock.id ]
        });
    });
    ...
    test("counts certification coverage by overlap even when assignment is anchored to another segment block", () => {
        const day = createScheduleDay("day-cert-overlap", { dayOfWeek: "mon", dayScheduleType: "full_day" });
        const openBlock = createSegmentBlock("block-cert-open", {
            dayOfWeek: "mon",
            segment: "open",
            startTime: "06:30",
            endTime: "10:15"
        });
        const midBlock = createSegmentBlock("block-cert-mid", {
            dayOfWeek: "mon",
            segment: "mid",
            startTime: "10:15",
            endTime: "14:15"
        });
        const employee = createEmployee("emp-cert-overlap", {
            medicallyDelegated: true,
            cprCurrent: true
        });
        const mergedStyleAssignment = createAssignment("assign-cert-overlap", openBlock.id, employee.id, {
            startTime: "06:30",
            endTime: "14:15"
        });
        const context: RulesContext = withDefaultScheduleInfo({
        ...
        const violations = engine.evaluate(context);
        const certificationViolations = violations.filter((violation) => violation.ruleId === "certification-per-segment");
        expect(certificationViolations).toHaveLength(0);
    });
